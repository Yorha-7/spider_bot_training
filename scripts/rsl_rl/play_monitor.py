...
"""Script to play a checkpoint while monitoring commanded velocity & IMU acceleration."""
...
parser = argparse.ArgumentParser(description="Play a checkpoint with fixed velocity commands and monitor IMU data.")
...
parser.add_argument("--print_interval", type=int, default=25, help="Print monitor output every N steps (default: 25)")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    """Play with RSL-RL agent while monitoring commands & IMU."""
    ...
    # values instead of sampling random commands.
    ...
    print(f"[INFO] Printing monitor output every {args_cli.print_interval} steps")

    # wrap for video recording
    if args_cli.video:
        video_kwargs = {
            "video_folder": os.path.join(log_dir, "videos", "play_monitor"),
    ...
    while simulation_app.is_running():
        ...
        # --- Monitor output ---
        if timestep % args_cli.print_interval == 0:
            cmd = env.unwrapped._commands[0]
            imu = env.unwrapped.scene["imu"]
            acc = imu.data.lin_acc_b[0]
            vel = env.unwrapped._robot.data.root_lin_vel_b[0]
            print(
                f"[step {timestep:4d}] "
                f"cmd: vx={cmd[0]:+.3f} vy={cmd[1]:+.3f} wz={cmd[2]:+.3f}  |  "
                f"acc: ax={acc[0]:+.3f} ay={acc[1]:+.3f} az={acc[2]:+.3f}  |  "
                f"vel: vx={vel[0]:+.3f} vy={vel[1]:+.3f} vz={vel[2]:+.3f}",
                flush=True,
            )
